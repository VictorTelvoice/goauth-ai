...
Busca un número libre en el inventario global de Telsim y lo asigna a tu cuenta.

* **Metodo**: `POST`
* **Ruta**: `/numbers`
* **Cuerpo (JSON)**:
    * `region`: (String) Código de región (Ej: "US"). Por defecto "US".
    * `plan_type`: (String) Tipo de plan (Ej: "basic", "premium"). Por defecto "basic".
    * `label`: (String) Etiqueta personalizada para identificar el número.
